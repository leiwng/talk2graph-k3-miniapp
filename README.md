...
> **在线试用（W8 内测）**：`http://<部署后填IP>:8080`（部署完成后由运维人填回此处）

- **后端**：Python + FastAPI + SymPy + scipy（约束求解）
- **前端**：React + TypeScript + Vite + Zustand + SVG
- **LLM**：火山方舟 GLM-5.2 / DeepSeek v4-flash / MiniMax-M3 / 智谱（可切换；当前默认火山方舟 GLM-5.2）
- **存储**：SQLite（免登录，会话持久化；老师反馈持久化）
- **MVP 范围**：初中平面几何（三角形、圆、四边形、相关约束）
- **不在 MVP 范围**：函数图像 / 圆锥曲线（抛物线/椭圆/双曲线）/ 立体几何 / 统计图表 — AI 会主动拒绝并给出友好提示

## 当前进度

| 里程碑 | 状态 | 说明 |
|---|---|---|
| W1 — DSL + 求解器 + SVG 渲染 | ✅ | 5 个端到端测试通过 |
| W2 — LLM 抽象层（多 Provider）+ Prompt | ✅ | 9 个测试；12 条中文 few-shot 全部可解 |
| W3 — DSL diff + 会话 + API | ✅ | 12 个测试；完整 chat→undo→redo→export 流程跑通 |
| W4 — 前端 MVP | ✅ | React + Vite，三栏布局可用 |
| W5 — 扩展约束 + 渲染装饰 + 交互 | ✅ | 9 个测试；中点/垂足/角平分线/共圆/平行四边形 + 直角小方块/等长刻度/角度弧 + 画板 hover/拖动 |
| W6 — 内测打磨 + Docker 部署 | ✅ | 30 个测试（10 ops + 20 压测）；中文错误分类、admin stats、Docker Compose、腾讯云部署脚本、老师手册 |
| W7 — 试用前发布打磨 | ✅ | 11 个测试；输入即时显示、LLM 拒绝按类型分色、👍/👎 反馈按钮、Feedback 表 + JSONL 导出 |
| W8 — 生产部署（腾讯云轻量服务器 + COS 备份） | 🚀 | 部署脚本端口参数化、幂等不删 DB、运维 SOP、安全组清单；待 SSH 落地后填实际 IP |
